@extends('front-end/app/app-home-asset', [
    'Clients ',
    'body_class' => 'vertical bg-secondary1/5 dark:bg-bg3
my-products-page',
])
...
@section('content')
    ...
    <main class="main-content has-sidebar">
                ...
                <div class="row flex items-center justify-end gap-3">
                    ...
                    <div class="content-right">
                        <div class="flex gap-2">
                            <a href="{{ route('releve-client', 'fcp') }}"
                                class="btn {{ $type == 'fcp' ? 'btn-primary' : 'btn-outline-primary' }}"
                                style="padding: 8px 15px; {{ $type == 'fcp' ? 'background-color: #0066cc;' : 'color: #0066cc; border-color: #0066cc;' }}">Relevés
                                FCP</a>
                            <a href="{{ route('releve-client', 'pmg') }}"
                                class="btn {{ $type == 'pmg' ? 'btn-primary' : 'btn-outline-primary' }}"
                                style="padding: 8px 15px; {{ $type == 'pmg' ? 'background-color: #C49A22;' : 'color: #C49A22; border-color: #C49A22;' }}">Relevés
                                PMG</a>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-hover align-middle" border="1">
                    <thead class="table-light">
                        <tr>
                            ...
                            @if ($type == 'fcp' || $type == 'all')
                                <th style="padding: 10px;">Relevé FCP</th>
                            @endif
                            @if ($type == 'pmg' || $type == 'all')
                        ...
                        @forelse ($clients as $client)
                            <tr>
                                ...
                                @if ($type == 'fcp' || $type == 'all')
                                    <td align="center">
                                        @if ($client->has_fcp)
                                            <button class="btn btn-sm btn-outline-info btn-preview preview-link-fcp"
                                                data-id="{{ $client->id }}" data-type="fcp">
                                                Prévisualiser
                                            </button>
                                        @else
        ...
        <div class="mt-4 flex justify-end gap-3">
            @if ($type == 'fcp' || $type == 'all')
                <button type="button" id="btnSendSelectedFcp" class="btn btn-primary btn-type-send" data-type="fcp"
                    disabled style="background-color: #0066cc; border-color: #0066cc;">
                    Générer et envoyer les relevés FCP
                </button>
            @endif

            @if ($type == 'pmg' || $type == 'all')
                <button type="button" id="btnSendSelectedPmg" class="btn btn-primary btn-type-send" data-type="pmg"
                    disabled style="background-color: #C49A22; border-color: #C49A22;">
                    Générer et envoyer les relevés PMG
                </button>
            @endif
        </div>
    ...
    <div id="confirmSendPopup" class="mfp-hide white-popup-block">
        ...
        Vous êtes sur le point d’envoyer les relevés <strong id="confirmType"></strong> de
...
@section('script_front_end')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const checkAll = document.getElementById('checkAllClients');
            const sendBtns = document.querySelectorAll('.btn-type-send');
            ...
            function updateButtonState() {
                const count = getSelectedClients().length;
                sendBtns.forEach(btn => btn.disabled = count === 0);
            }
    ...
    <script>
        $(document).ready(function() {
            ...
            // 🔹 Boutons par type
            $('.btn-type-send').on('click', function() {
                ...
                $('#confirmType').text(currentType.toUpperCase());
            ...
            $('#btnConfirmSend').on('click', function() {
                ...
                $btn.prop('disabled', true)
                    .html(
                        '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Envoi en cours...'
                        );
